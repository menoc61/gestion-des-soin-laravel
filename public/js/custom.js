...
(function ($) {
    ...
    $("#RdvDetailModal").on("show.bs.modal", function (event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        var rdv_date = button.data("rdv_date"); // Extract info from data-* attributes
        var rdv_id = button.data("rdv_id"); // Extract info from data-* attributes
        var rdv_time_start = button.data("rdv_time_start"); // Extract info from data-* attributes
                ...
                success: function (array) {
                    ...
                    $.each(array, function (key, value) {
                        if (value.available == "available") {
                            options =
                                options +
                                '<div class="col-sm-6 col-md-4 mb-2"><button class="btn btn-doctorino btn-block" data-toggle="modal" data-target="#RDVModalSubmit" data-rdv_date="' +
                                date +
                                '" data-rdv_time_start="' +
                                value.start +
                                '" data-rdv_time_end="' +
                            ...
                            options =
                                options +
                                '<div class="col-sm-6 col-md-4 mb-2"><button class="btn btn-danger btn-block">' +
                                value.start +
                                " - " +
                                value.end +
                                "</button></div>";
                    ...
                    if (!options) {
                        ...
                        $("#help-block").html(
                            "<img src='" +
                                baseUrl +
                                "img/rest1.png' alt='Image'/> <br> <b>Désolé, le praticien ne travaille pas ce jour-là</b>"
                        );
                    }
                    ...
                    $("#RDVModalSubmit").on("show.bs.modal", function (event) {
                        ...
                        var selectedPatientName = $(
                            "#patient_name option:selected"
                        ).text(); // Extract info from data-* attributes
                        ...
                        if (selectedPatientID == 0) {
                            modal
                                .find("#patient_name")
                                .html(
                                    '<span class="text-danger"><b>Sélectionnez le patient avant de soumettre</b></span>'
                                );
                        } else {
                            modal
                                .find("#patient_name")
                        ...
                        modal
                            .find("#rdv_time")
                            .text(rdv_time_start + " - " + rdv_time_end);
                        modal.find("#rdv_time_start_input").val(rdv_time_start);
                        modal.find("#rdv_time_end_input").val(rdv_time_end);
                        modal.find("#patient_input").val(selectedPatientID);
